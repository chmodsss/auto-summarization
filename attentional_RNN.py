...
#%%
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            saver.save(sess, "/home/kloe_mr/AnacondaProjects/tf-rnn-attention/Bachelorarbeit/BA/buffering-model-rnn.ckpt")	
            # Testing
            print(X_test.shape)
...
#%% Restore session 
        with tf.Session() as sess:
            ...
            for each_y_batch in y_batch_train:
                ...
                y_batch_normed.append(norm_y)
                j += 1
            alphas_train = sess.run([alphas], feed_dict={batch_ph: x_batch_train, target_ph: y_batch_normed,
...
#%% Restore session 
        with tf.Session() as sess:
            ...
            #saver.restore(sess, "H:/AnacondaProjects/tf-rnn-attention/tf-rnn-attention-master/model.ckpt")#"model")
            max_y = max(max(len(x) for x in train_summaries), max(len(x) for x in test_summaries))+2
            x_batch_test, y_batch_test = X_test[:len(X_test)], y_tests[:len(y_tests)]
            ...
            for each_y_batch in y_batch_test:
                ...
                y_batch_normed.append(norm_y)
                j += 1
            alphas_test = sess.run([alphas], feed_dict={batch_ph: x_batch_test, target_ph: y_batch_normed,
                                                        seq_len_ph: seq_len_test, keep_prob_ph: 1.0})
    ...
    def rouge_score(rnn_outs, rnn_out, batch_words):
        ...
        for i in range(len(rnn_outs)):
            ...
            try:
                ...
                continue 
            d.append((batch_words[i], rnn_out[i], precision, recall, f_score, t, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, KEEP_PROB, BATCH_SIZE, NUM_EPOCHS, DELTA, LEARNING_RATE, ALPHA_DIVIDER, MAXIMUM_DATA_NUM, mean_testloss, avg_testloss, std_testloss))
            print("Summary" + str(i) + "\nPrecision is :"+str(precision)+"\nRecall is :"+str(recall)+"\nF Score is :"+str(f_score))
        return d
    
    d = rouge_score(rnn_outs, rnn_out, batch_words)
    df = pd.DataFrame(d, columns=('Original Sum', 'Predicted Sum', 'Precision', 'Recall', 'F_Score', 'RUNTIME/sec', 'EMBEDDING_DIM', 'HIDDEN_SIZE', 'ATTENTION_SIZE', 'KEEP_PROB', 'BATCH_SIZE', 'EPOCHS', 'DELTA', 'LEARNING_RATE', 'ALPHA_DIVIDER', 'MAXIMUM_DATA_NUM', 'MEAN_TESTLOSS', 'AVG_TESTLOSS','STD_TESTLOSS'))
...
#%%
    ...
    print(df.head(2))
    df.to_csv("arnn_test_1" + str(BATCH_SIZE) + "_batches" + str(NUM_EPOCHS) + "_epochs.csv", sep=',')  
